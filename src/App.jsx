...
const SUPABASE_KEY = "sb_publishable_oq4LlS4VRLWXBY2zfEcj-A_iUilGZFe";
...
function pinColor(id) {
  const colors = ["#22E88C", "#06B6D4", "#F59E0B", "#EAF6EF"];
  const str = String(id);
  let hash = 0;
  for (let i = 0; i < str.length; i++) hash = (hash * 31 + str.charCodeAt(i)) % 1000;
...
:root{
  ...
  --sage:#22E88C;
  --sage-l:#0F2A1E;
  --line:rgba(34,232,140,0.22);
  --shadow-pin: 0 3px 0 rgba(0,0,0,0.5), 0 8px 26px rgba(34,232,140,0.14);
}
body{
  ...
  min-height:100vh;
  background-image:
    radial-gradient(circle at 15% 20%, rgba(34,232,140,0.08) 0%, transparent 35%),
    radial-gradient(circle at 85% 15%, rgba(6,182,212,0.08) 0%, transparent 35%),
    radial-gradient(circle at 50% 90%, rgba(34,232,140,0.05) 0%, transparent 40%);
...
.lh-nav{
  ...
  box-shadow:0 1px 20px rgba(34,232,140,0.08);
}
.lh-logo{
  ...
  text-shadow:0 0 18px rgba(34,232,140,0.35);
  animation:logoGlow 3s ease-in-out infinite;
}
@keyframes logoGlow{
  0%,100%{ text-shadow:0 0 14px rgba(34,232,140,0.3); }
  50%{ text-shadow:0 0 26px rgba(34,232,140,0.55), 0 0 44px rgba(34,232,140,0.2); }
}
.lh-logo span{
  background:linear-gradient(90deg,var(--red),var(--mustard),var(--red));
  background-size:200% auto;
  -webkit-background-clip:text;background-clip:text;color:transparent;
  filter:drop-shadow(0 0 12px rgba(34,232,140,0.6));
  animation:logoSheen 3.5s linear infinite;
}
@keyframes logoSheen{
...
.lh-badge{
  ...
  font-weight:600;
  letter-spacing:.02em;
  box-shadow:0 0 0 0 rgba(34,232,140,0.5);
  animation:badgeGlow 2.4s ease-in-out infinite;
}
@keyframes badgeGlow{
  0%,100%{ box-shadow:0 0 0 0 rgba(34,232,140,0.4); }
  50%{ box-shadow:0 0 0 7px rgba(34,232,140,0); }
}
...
.lh-hero::before{ background:rgba(34,232,140,0.20); top:-100px; left:-60px; }
...
.lh-cta-primary{
  ...
  color:#04140C;border:none;border-radius:6px;
  font-family:'Archivo Black',sans-serif;font-size:.92rem;text-transform:uppercase;letter-spacing:.02em;
  cursor:pointer;
  box-shadow:0 0 24px rgba(34,232,140,0.4);
  transition:transform .15s, box-shadow .25s;
}
.lh-cta-primary:hover{ transform:translateY(-2px); box-shadow:0 6px 30px rgba(34,232,140,0.55); }
.lh-cta-secondary{
  padding:15px 28px;
...
.lh-cta-secondary:hover{ border-color:var(--red); transform:translateY(-2px); }
.lh-counter{
  ...
  margin-bottom:28px;
  box-shadow:0 0 16px rgba(34,232,140,0.25);
...
.lh-tab.active{ background:linear-gradient(90deg,var(--red),var(--red-d)); color:#04120A; border-color:var(--red); box-shadow:0 4px 18px rgba(34,232,140,0.4); }
...
.lh-input{
  ...
  font-family:'Archivo', sans-serif;font-size:.88rem;color:var(--ink);background:var(--paper-dark);
  outline:none;transition:border-color .2s, box-shadow .2s;
}
.lh-input:focus{ border-color:var(--red); box-shadow:0 0 0 3px rgba(34,232,140,0.18); outline:none; }
.lh-input-error{ border-color:#FF6B6B !important; box-shadow:0 0 0 3px rgba(255,107,107,0.15) !important; }
.lh-tab:focus-visible, .lh-apply:focus-visible, .lh-submit:focus-visible, .lh-select:focus-visible { outline:2px solid var(--red); outline-offset:2px; }
.lh-search-input{padding-left:38px;}
.lh-select{
  padding:11px 14px;border:1px solid var(--line);border-radius:6px;
  font-family:'Archivo', sans-serif;font-size:.85rem;color:var(--ink);background:var(--paper-dark);
  cursor:pointer;outline:none;transition:border-color .2s;
}
.lh-select:focus{ border-color:var(--red); }
...
.lh-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:0;margin-bottom:30px;border:1px solid var(--line);border-radius:6px;overflow:hidden;}
.lh-stat{background:var(--card);padding:16px 10px;text-align:center;border-right:1px solid var(--line);}
.lh-stat:last-child{border-right:none;}
.lh-stat-num{font-family:'Archivo Black', sans-serif;font-size:1.5rem;color:var(--red);line-height:1;text-shadow:0 0 12px rgba(34,232,140,0.35);}
...
.lh-card{
  ...
  transition:transform .12s ease-out, box-shadow .25s ease, border-color .25s ease;
...
.lh-card::before{
  ...
  box-shadow: 0 2px 3px rgba(0,0,0,0.5), inset 0 1px 2px rgba(255,255,255,0.25);
  z-index:2;
}
.lh-card:hover{ box-shadow:0 0 0 1px var(--red), 0 14px 36px rgba(34,232,140,0.25); border-color:var(--red); }
.lh-card.lh-card-new{
...
.lh-form-card{ background:var(--card); border:1px solid var(--line); padding:30px 26px; box-shadow:0 0 0 1px var(--line), 0 20px 50px rgba(34,232,140,0.1); }
...
.lh-error{margin-top:14px;padding:10px 14px;background:rgba(255,90,90,0.1);border:1px solid #FF6B6B;font-size:.82rem;color:#FF9B9B;font-weight:600;}
.lh-submit{
  ...
  cursor:pointer;transition:box-shadow .25s, transform .15s;box-shadow:0 0 0 1px var(--line);
}
.lh-submit:hover{ box-shadow:0 6px 20px rgba(34,232,140,0.5); transform:translateY(-1px); }
...
.lh-toast{
  ...
  box-shadow:0 8px 24px rgba(34,232,140,.35);
...
.lh-modal{background:var(--card);border:1px solid var(--line);max-width:540px;width:100%;max-height:90vh;overflow-y:auto;padding:32px;box-shadow:0 0 0 1px rgba(34,232,140,0.3), 0 30px 80px rgba(34,232,140,0.15);animation:modalIn .3s cubic-bezier(.22,1,.36,1);}
.lh-modal-close{float:right;background:none;border:1px solid var(--line);width:30px;height:30px;font-size:1.1rem;cursor:pointer;line-height:1;margin-top:-4px;transition:background .2s,color .2s,transform .2s;}
...
.lh-modal-apply{display:block;width:100%;margin-top:24px;padding:14px;background:linear-gradient(90deg,var(--red),var(--red-d));color:var(--paper);border:none;font-family:'Archivo Black',sans-serif;font-size:.9rem;text-transform:uppercase;cursor:pointer;box-shadow:0 0 0 1px var(--line);transition:box-shadow .25s, transform .15s;}
.lh-modal-apply:hover{ box-shadow:0 6px 20px rgba(34,232,140,0.5); transform:translateY(-1px); }
...
export default function App() {
  ...
  return (
    <>
      <style>{css}</style>
      {isAdmin && (
        <div style={{background:"#0A0F0D",color:"#EAF6EF",padding:"20px",fontFamily:"monospace",borderBottom:"1px solid rgba(34,232,140,0.25)"}}>
          <h2 style={{color:"#22E88C",marginBottom:"16px"}}>📊 Admin Dashboard</h2>
          ...
          <hr style={{margin:"16px 0",borderColor:"rgba(34,232,140,0.25)"}}/>
          <h3 style={{color:"#22E88C",marginBottom:"10px"}}>Recent Listings</h3>
          {jobs.slice(0,5).map(j=><div key={j.id} style={{marginBottom:"8px",padding:"8px",background:"#121B17",border:"1px solid rgba(34,232,140,0.15)"}}><strong>{j.title}</strong> — {j.company} ({j.type})</div>)}
        </div>
      )}
      ...
      <section className="lh-hero">
        ...
        <h1>Find work.<br /><em>Hire local. Instantly.</em></h1>
        <p className="lh-hero-sub">Connecting local talent with local employers — no middlemen, no fees, no waiting.</p>

        <div className="lh-value-props">
          <div className="lh-value-chip">⚡ <span>Instant WhatsApp contact</span></div>
          <div className="lh-value-chip">🆓 <span>100% free, always</span></div>
          <div className="lh-value-chip">📍 <span>Real local jobs only</span></div>
          <div className="lh-value-chip">✅ <span>No sign-up needed</span></div>
        </div>

        <div className="lh-hero-cta">
          <button className="lh-cta-primary" onClick={() => setTab("post")}>📌 Post a Job — It's Free</button>
          <button className="lh-cta-secondary" onClick={() => setTab("browse")}>🔍 Browse Jobs</button>
        </div>
